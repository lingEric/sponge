...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        vector<GroupInformation> groups = [&]() {
            if (!group_id.empty()) {
                return vector<GroupInformation>{
                    producer.get_consumer_group(group_id)};
            } else {
        ...
        for (const GroupInformation& group : groups) {
            ...
            for (const GroupMemberInformation& info : group.get_members()) {
                ...
                if (show_assignment) {
                    MemberAssignmentInformation assignment(
                        info.get_member_assignment());
                    cout << " has assigned: "
                         << assignment.get_topic_partitions();
                }
                cout << endl;
            }
            cout << endl;
